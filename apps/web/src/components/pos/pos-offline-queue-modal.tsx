...
export function PosOfflineQueueModal({
  ...
  async function discard(id: string) {
    if (!window.confirm(t("confirmDiscardOfflineSale"))) return;
    setBusyId(id);
  ...
  function statusLabel(status: QueuedSale["status"]) {
    if (status === "failed") return t("queueStatusFailed");
    if (status === "syncing") return t("queueStatusSyncing");
        ...
        <div className="pos-header flex items-center justify-between px-5 py-4">
          <div>
            ...
            <p className="text-xs text-white/70">
              {t("offlineQueuePendingFailed", { pending: pending.length, failed: failed.length })}
            </p>
          </div>
          <button
            type="button"
            ...
            aria-label={t("closeOfflineSyncQueue")}
          >
        ...
        <div className="flex-1 overflow-y-auto p-4" aria-live="polite">
          ...
          {!loading && items.length === 0 && (
            <p className="rounded-xl border border-slate-200 bg-slate-50 p-6 text-center text-sm text-slate-500">
              {t("noOfflineSalesWaiting")}
            </p>
          )}

          <ul className="space-y-2">
            {items.map((item) => {
              const est = formatCurrency(
