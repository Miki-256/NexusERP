-- Pilot: enqueue pos.sale_completed notification after checkout.
...
BEGIN
  ...
  PERFORM public.enqueue_notification_event(
    ...
    jsonb_build_object(
      ...
      'total', v_total,
      'customer_name', COALESCE(NULLIF(trim(COALESCE(p_customer_name, v_cust_name)), ''), 'Walk-in'),
      'customer_phone', COALESCE(p_customer_phone, v_cust_phone),
      'store_id', p_store_id,
      'register_id', p_register_id,
      'cashier_id', v_staff_id,
      'payments_pending', v_has_pending_payments,
      'link', '/sales/' || v_sale_id::text
    ),
